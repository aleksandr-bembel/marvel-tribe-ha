...
│   ├── number.py                       # Числовые настройки
│   ├── select.py                       # Выбор режимов
...
- ✅ **Полное управление Ambient Light** (6 индикаторов, 4 эффекта, яркость 10-100%)
- ✅ **Управление LCD дисплеем** (яркость 0-100%, полное выключение)
- ✅ **Система будильников** (мониторинг 6 слотов, состояние системы)
- ✅ **Auto-Sleep режим** (включение/выключение, мониторинг периода)
- ✅ **Аудио система** (включение/выключение, громкость клавиш/запуска/будильника)
- ✅ **WiFi мониторинг** (статус подключения, SSID, IP адрес)
- ✅ **Системная диагностика** (прошивка, язык, время последнего обновления)
- ✅ **Мгновенная синхронизация** всех элементов управления
...
4. Введите IP адрес ваших часов (например: `192.168.1.100`)
5. Нажмите **Отправить**
...
|----------|----------|--------------|
| **Host** | IP адрес часов Marvel Tribe | `192.168.1.100` |
...
- `sensor.marvel_tribe_rgb_brightness` - яркость ambient light (сенсор)
...
### 🌈 Ambient Light управление

```yaml
# Включение Ambient Light с настройкой яркости
automation:
  - alias: "Marvel Tribe - Вечерняя подсветка"
    trigger:
      - platform: sun
        event: sunset
    action:
...
**Решение:**
1. Проверьте IP адрес в веб-интерфейсе часов или роутера
2. Убедитесь, что часы подключены к сети
3. Проверьте доступность: `ping IP_АДРЕС_ЧАСОВ`
